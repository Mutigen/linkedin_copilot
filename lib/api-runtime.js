...
function buildWarmSignal(body) {
  ...
  const stage = String(body.stage || '').trim() || 'DM an warmen Lead nach Signal oder Antwort';
...
function formatCommentResponse(result) {
  ...
  return {
    ...
    message: commentDrafts.length > 0
      ...
      : 'Kein Kommentar erzeugt. Prüfe, ob Claude aktiv ist und der eingefügte Post Text enthält.',
  };
}
...
async function generateCommentFromBody(body) {
  ...
  const commentDepth = String(body.commentDepth || '').trim().toLowerCase() === 'x2' ? 'x2' : 'normal';
  if (!postText) {
